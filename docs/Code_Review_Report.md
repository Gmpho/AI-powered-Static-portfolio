...
*   **Purpose:** The main client-side application logic file. It's responsible for initializing the UI, rendering dynamic content (projects, chat history), handling user interactions (chat submission, theme toggling), and orchestrating communication with the Cloudflare Worker via `chatbot.ts`. It acts as the "view" and "controller" for the frontend.
*   **Key Components:**
    *   **Imports:** `sendPrompt` (communication), `marked`, `hljs` (Markdown/syntax highlighting), `stateService`, `ChatMessage` (state management), `projects` (static data), `DOMPurify` (XSS sanitization).
    *   **DOM References:** Selects various HTML elements by their IDs.
    ...
    *   **`displayContactForm()`:** Dynamically creates and displays a contact form within the chat window upon worker signal.
    *   **Theme Toggling:** `setTheme()`, `handleThemeToggle()` manage light/dark mode persistence.
    *   **State Subscription:** `stateService.subscribe()` reacts to global state changes to update the UI and persist chat history.
    *   **`handleChatSubmit()`:** Processes user input, adds to state, calls `sendPrompt`, and manages streaming callbacks (`onStreamChunk`, `onStreamComplete`, `onError`).
    *   **Initialization:** `DOMContentLoaded` listener sets up initial UI, loads history/theme, and attaches event listeners.
*   **Logic Flow:** Event-driven UI updates based on `stateService` changes. Asynchronous communication with worker via `sendPrompt`.
*   **Interactions:** `chatbot.ts` (communication), `stateService.ts` (state management), `projects.ts` (data), `index.css` (styling), Cloudflare Worker (indirectly).
*   **Best Practices:** Vanilla JS/TS, reactive state management, robust SSE parsing, XSS prevention via `DOMPurify`, modularity.
...
*   **Key Components:**
    ...
    *   **`sendPrompt()`:**
        *   **Config Check:** Validates `VITE_WORKER_URL`.
        ...
        *   **Callbacks:** Uses `onStreamChunk`, `onStreamComplete`, `onError` to report progress to `frontend/index.ts`.
...
*   **Best Practices:** Separation of concerns, robust SSE parsing, callback-based async communication, Gemini API compatibility.
...
*   **Key Components:**
    ...
    *   **`TRIPWIRE` (Regex):** Aggressive regex to detect command execution keywords, sensitive file paths, API keys/secrets, and shell metacharacters/command substitution.
    *   **`sanitizeOutput(text)`:** Strips `<script>` tags, all HTML, redacts long `data:` URIs/base64, and API key-like tokens from AI responses.
...
*   **Purpose:** Implements the `displayContactForm` tool, signaling the frontend to show a contact form.
*   **Key Components:**
...
*   **Best Practices:** Standard Vite configuration, GitHub Pages compatibility, clean builds.
...
*   **Key Components:**
    ...
    *   **`jobs`:**
        ...
        *   **`deploy`:**
            ...
            *   Builds frontend.
            *   **`Run Playwright tests`:** Executes E2E tests as a quality gate.
            *   Deploys frontend to GitHub Pages.
*   **Logic Flow:** Sequential execution of jobs: worker deployment, then frontend build, test, and deployment.
*   **Interactions:** Relies on `package.json` scripts, `vite.config.ts`, `playwright.config.ts`, `wrangler-action`, GitHub Secrets.
*   **Best Practices:** Full CI/CD automation, separate build/deploy jobs, **test gate (Playwright E2E)**, environment variables/secrets, artifact management.
*   **Considerations:** Worker unit tests in CI, linting/type checking steps, security scanning.

---

### **17. `GEMINI.md`**

*   **Purpose:** Project documentation, providing an overview, architecture, technology stack, security details, development conventions, and troubleshooting.
*   **Key Components:**
    ...
    *   **Security Enhancements: Distributed Rate Limiting with Cloudflare KV:** (Recently updated to reflect KV-backed implementation).
